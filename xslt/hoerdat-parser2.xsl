...
http://www.hoerdat.in-berlin.de/index.php
...
<xsl:transform
    ...
    <xsl:template match="html:table[html:colgroup]">
        <Sendung>
    ...
    <xsl:template match="html:td" mode="value">
        ...
        <xsl:choose>
            <xsl:when test="html:b/html:a">
                <xsl:value-of select="normalize-space(html:b/html:a/text())"/>
            </xsl:when>
            <xsl:when test="html:b">
                <xsl:value-of select="normalize-space(html:b/text())"/>
            </xsl:when>
            <xsl:when test="$key = 'Mitwirkende'">
                <xsl:apply-templates select="descendant::html:td[@class = 'mit']" mode="mitwirkende"/>
            </xsl:when>
    ...
    <xsl:template match="html:td" mode="mitwirkende">
